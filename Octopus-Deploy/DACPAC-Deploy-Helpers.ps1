...
function Apply-ConnectionString-PublishProfile {
	...
	$pubFile = $(Get-ChildItem . -Recurse | ? { $_.Name -match ".InvasiveMode." }).FullName
...
function Perform-Dacpac-Publish {
	...
	if($InvasiveMode) {
		...
		$publishProfileLocation = $(Get-ChildItem . -Recurse | ? { $_.Name -match ".InvasiveMode." }).FullName
	}  else  {
		Write-Output "Performing DACPAC publish in 'Strict mode'"
		$publishProfileLocation = $(Get-ChildItem . -Recurse | ? { $_.Name -match ".StrictMode." }).FullName
	}

	$dacpacLocation = $(Get-ChildItem . | ? { $_.Name -imatch "$dacpacPackageName.dacpac" }).FullName
