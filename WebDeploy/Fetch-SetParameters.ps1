...
Function Find-SettingsFileOnStash {
	param(
		[string]$TargetEnvironment,
		[string]$AppName,
		[string]$FilesStashRestUrl)

	return (Invoke-RestMethod -Uri $FilesStashRestUrl -Method Get).values | Where-Object { $_ -match $AppName -and $_ -match $TargetEnvironment }
}

Function Download-SettingsFileFromStash {
	param(
		[string]$settingsFile,
...
$settingsMatch = Find-SettingsFileOnStash $TargetEnvironment $AppName $StashFilesRestUrl
...
if($AllowDefaultSettings -and ($settingsMatch.Count -eq 0)) {
	...
	$settingsMatch = Find-SettingsFileOnStash 'localhost' $AppName $StashFilesRestUrl
}
...
$defaultSettingsFile = (Get-Item $PackageLocation).GetFiles() | Where-Object { $_.Name -match 'SetParameters.xml' }
...
Download-SettingsFileFromStash $settingsMatch $StashSettingsLocationUrl $PackageLocation
	

# rename the targetd SetParameters file
$newSettingsFile = (Get-Item $PackageLocation).GetFiles() | Where-Object { $settingsMatch.EndsWith($_.Name)  }
Write-Host "Renaming environment specific '*SetParameters.xml' file with '$newSettingsFile' to '$defaultSettingsFile'"
Move-Item $newSettingsFile.FullName $defaultSettingsFile.FullName