...
param([string]$targetServer, [switch]$interActiveMode)

Function GetWebDeployCmdFile {
    # Get Application WebDeploy cmd file
    $webDeployCmdMatch = (Get-Item .).GetFiles() | Where { $_.Name.EndsWith(".cmd") }
...
Function VerifySetParametersFileExist {
    # Check if 'SetParameters file exist'
    $setParamMatch = (Get-Item .).GetFiles() | Where { $_.Name -match "SetParameters.xml" }
...
Function ExecuteWebDeploy {
    ...
    if(($userConfirm -match "y") -eq $false) {
        ...
        if(($cmdOutput | ? { $_.GetType() -match "System.Management.Automation.ErrorRecord" })) {
...
Function GetServers
{
    $matches = (Get-Item .).GetFiles() | Where { $_.Name -match "Servers.txt" }
...
Try
{
    ...
    $webDeployCmd = GetWebDeployCmdFile
    ...
    while($serverIdx -lt $serverCount)
    {
        ...
        ExecuteWebDeploy $webDeployCmd $targetServerName -interActiveMode:$interActiveMode
    ...
    while($askForNewDeploy -eq $true) {
        ...
        if(($userWantsToProceed -match "y") -eq $false) {   
            ...
            ExecuteWebDeploy $webDeployCmd $desiredServer
        }
    }
